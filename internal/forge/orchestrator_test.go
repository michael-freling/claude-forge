...
func TestRepairWorktreeLinks(t *testing.T) {
	t.Run("converts relative gitdir to absolute", func(t *testing.T) {
		projectDir := t.TempDir()

		// Create worktree directory structure mimicking what the container creates
		wtName := "test-worktree"
		worktreeDir := filepath.Join(projectDir, ".claude", "worktrees", wtName)
		gitWorktreeDir := filepath.Join(projectDir, ".git", "worktrees", wtName)
		require.NoError(t, os.MkdirAll(worktreeDir, 0o755))
		require.NoError(t, os.MkdirAll(gitWorktreeDir, 0o755))

		// Write relative gitdir in .git/worktrees/<name>/gitdir
		relGitdir := "../../../.claude/worktrees/" + wtName + "/.git"
		require.NoError(t, os.WriteFile(
			filepath.Join(gitWorktreeDir, "gitdir"),
			[]byte(relGitdir+"\n"), 0o644,
		))

		// Write the worktree's .git file with relative path back
		worktreeGitFile := filepath.Join(worktreeDir, ".git")
		relBack := "gitdir: ../../../.git/worktrees/" + wtName
		require.NoError(t, os.WriteFile(worktreeGitFile, []byte(relBack+"\n"), 0o644))

		repairWorktreeLinks(projectDir)

		// Verify .git/worktrees/<name>/gitdir is now absolute
		content, err := os.ReadFile(filepath.Join(gitWorktreeDir, "gitdir"))
		require.NoError(t, err)
		got := strings.TrimSpace(string(content))
		assert.True(t, filepath.IsAbs(got), "expected absolute path, got %q", got)
		assert.Equal(t, worktreeGitFile, got)

		// Verify worktree's .git file is now absolute
		wtContent, err := os.ReadFile(worktreeGitFile)
		require.NoError(t, err)
		wtGot := strings.TrimSpace(string(wtContent))
		assert.True(t, strings.HasPrefix(wtGot, "gitdir: /"), "expected absolute gitdir, got %q", wtGot)
		expectedBack := "gitdir: " + gitWorktreeDir
		assert.Equal(t, expectedBack, wtGot)
	})

	t.Run("skips already absolute paths", func(t *testing.T) {
		projectDir := t.TempDir()
		...
		worktreeGitFile := filepath.Join(worktreeDir, ".git")
		require.NoError(t, os.WriteFile(worktreeGitFile, []byte("gitdir: "+gitWorktreeDir+"\n"), 0o644))

		absGitdir := worktreeGitFile
		require.NoError(t, os.WriteFile(
			filepath.Join(gitWorktreeDir, "gitdir"),
			[]byte(absGitdir+"\n"), 0o644,
		))
		...
		content, err := os.ReadFile(filepath.Join(gitWorktreeDir, "gitdir"))
		require.NoError(t, err)
		assert.Equal(t, absGitdir+"\n", string(content))
	})

	t.Run("no worktrees dir is a no-op", func(t *testing.T) {
		projectDir := t.TempDir()
		repairWorktreeLinks(projectDir) // should not panic
	})

	t.Run("skips entries where target does not exist", func(t *testing.T) {
		projectDir := t.TempDir()

		gitWorktreeDir := filepath.Join(projectDir, ".git", "worktrees", "stale")
		require.NoError(t, os.MkdirAll(gitWorktreeDir, 0o755))

		// Relative path that points to a non-existent directory
		require.NoError(t, os.WriteFile(
			filepath.Join(gitWorktreeDir, "gitdir"),
			[]byte("../../../.claude/worktrees/stale/.git\n"), 0o644,
		))

		repairWorktreeLinks(projectDir)

		// File should be unchanged (still relative)
		content, err := os.ReadFile(filepath.Join(gitWorktreeDir, "gitdir"))
		require.NoError(t, err)
		assert.Equal(t, "../../../.claude/worktrees/stale/.git\n", string(content))
	})
}
